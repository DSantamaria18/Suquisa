...
if(!empty($id_category))
{
	...
	$sql="SELECT p.*, pl.* ".(version_compare(_PS_VERSION_, '1.5.0.0', '>=')?" , prs.* ":"").", cp.position ".(in_array("image", $cols)?" ,i.id_image ":''); 
	...
	$sql.="	 FROM "._DB_PREFIX_."product p
				...
				".(in_array("image", $cols)?" LEFT JOIN "._DB_PREFIX_."image i ON (i.id_product= p.id_product AND i.cover=1)":'');
