			...
			if(!empty($log_ids[$num]))
			{
				...
				if(!empty($action) && $action=="insert")
				{
					...
					if (SCMS)
					{
						...
						{
							if(!in_array($shop["id_shop"], $shops_cats))
							{
					...
					if(SCAS)
					{
						...
						if(!empty($type_default) && in_array($type_default, array(2,3)))
						{
				...
				elseif(!empty($action) && $action=="update" && !empty($gr_id))
				{
					...
					foreach($fields AS $field)
					{
						...
						{
							if (in_array($field,array('reduction_price','reduction_percent','reduction_from','reduction_to','reduction_tax','unit_price_ratio','price')) && version_compare(_PS_VERSION_, '1.4.0.0', '>='))
							{
								...
								foreach($res as $key => $row)
								{
									...
									foreach($row as $col => $val)
									{
										if ($col!='id_product' && $col!='id_product_attribute')
											$sqlinsert.='\''.psql($val).'\',';
										if ($col!='id_product' && $col!='id_product_attribute' && !in_array('`'.$col.'`',$cols))
											$cols[]='`'.$col.'`';
									}
					...
					if (isset($_POST['quantityupdate']) || isset($_POST['quantity']))
					{
						...
						if ($quantityUpdate!=0)
						{
							...
							if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
								...
								StockAvailable::updateQuantity($id_product, null, $quantityUpdate, $id_shop);
						}else{
					...
					foreach($fields_lang AS $field)
					{
						if (isset($_POST[$field]))
						{
							$value=psql(Tools::getValue($field),(in_array($field,$fieldsWithHTML)?true:false));
					...
					if (count($todo))
					{
						...
						if($doHookUpdateQuantity && isset($return_datas['newQuantity']))
						{
