 ...
 **/
	...
	function getRowsFromDB(){
		...
		// on r�cup�re les valeurs du produit s�lectionn�
		$sql = '
		SELECT fp.id_feature,fp.id_feature_value,fv.custom,fvl.value,fvl.id_lang
		FROM `'._DB_PREFIX_.'feature_product` fp
		...
		{
			$Fkey = $Fval['id'];
			if (count($featuresSelection) && !in_array($Fkey,$featuresSelection))
				continue;
			$options='<option value="-1">--</option><option value="-2">'._l('Custom').'</option>';
			if (in_array('values',array_keys($Fval)))
