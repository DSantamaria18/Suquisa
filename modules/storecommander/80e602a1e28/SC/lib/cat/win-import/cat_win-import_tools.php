 ...
 **/
	...
	function readImportConfigXML($files)
	{
		...
		{
			if ($file!='' && !in_array($file,array_keys($importConfig)) && strpos($file,'&')===false)
	...
	{
		if (in_array(Tools::strtoupper($value),array('1','YES','TRUE','VRAI','OUI','ON'))) return true;
		return false;
	}
	
	function getCategoryPath($id_category,$path='')
	{
		global $categoryNameByID,$categoriesProperties;
		if ($id_category>1)
		{
		...
		if ($create_categories >= 1)
		{
			if (is_array($categList))
			{
				...
				if (strpos($categList,'>')!=false)
				{
					...
					if (!in_array($categList,$categoriesFirstLevel))
					{
						$newCateg=new Category();
	...
	function fieldInMapping($field)
	{
		...
		foreach($line AS $k => $v)
		{
			if (in_array($firstLineData[$k],$mappingData['CSVArray']) && $mappingData['CSV2DB'][$firstLineData[$k]]==$field)
				$return = true;
		}
		return $return;
	}
	...
	function findCSVLineValue($valueToFind)
	{
		...
		foreach($line AS $k => $v)
		{
			if (!sc_array_key_exists($k,$firstLineData))
				return '';
			if (in_array($firstLineData[$k],$mappingData['CSVArray']) && $mappingData['CSV2DB'][$firstLineData[$k]]==$valueToFind)
				return $v;
		}
		return '';
	}
	...
	function findCSVLineValueByLang($valueToFind,$id_lang)
	{
		...
		foreach($line AS $k => $v)
		{
			if (in_array($firstLineData[$k],$mappingData['CSVArray']) && $mappingData['CSV2DB'][$firstLineData[$k]]==$valueToFind && intval($getIDlangByISO[$mappingData['CSV2DBOptions'][$firstLineData[$k]]])==$id_lang)
				return $v;
		}
		return '';
	}
	...
	function findAllCSVLineValue($valueToFind,&$arrayToFill,$optionToGet=null,$fromObject=null)
	{
		...
		foreach($line AS $k => $v)
		{
			...
			*/
			if (in_array($firstLineData[$k],$mappingData['CSVArray']) && sc_array_key_exists($firstLineData[$k],$mappingData['CSV2DB']) && $mappingData['CSV2DB'][$firstLineData[$k]]==$valueToFind)
			{
	...
	*/
			...
			if ($attributeValue!='')
			{
				...
				if ($av[0]!='' && $av[1]!='')
				{
					...
					if ($av[1]!='')
					{
						...
						$newAttributeValue->name[intval(Configuration::get('PS_LANG_DEFAULT'))]="new";
						...
						$sql2 = "UPDATE "._DB_PREFIX_."attribute_lang SET name='".pSQL(cleanQuotes(Tools::substr($av[1],0,255)), true)."' WHERE id_attribute=".intval($newAttributeValue->id)." AND id_lang=".intval(Configuration::get('PS_LANG_DEFAULT'));
	...
	function isCombination($lineToCheck=NULL)
	{
		...
		if ($id_product_attribute==0 && (in_array('attribute',$mappingData['DBArray']) || in_array('attribute_multiple',$mappingData['DBArray'])))
		{
			...
			if ($lineToCheck==NULL)
			{
				...
				{
					if (sc_array_key_exists($firstLineData[$key],$mappingData['CSV2DB']) && in_array($mappingData['CSV2DB'][$firstLineData[$key]],array('attribute','attribute_multiple')) && $val!='')
						return true;
				}
			}
		}
	...
	function isCombinationWithID()
	{
		...
		if (in_array('id_product_attribute',$mappingData['DBArray']) || $id_product_attribute > 0)
			return true;
		return false;
	}
	...
	function refreshCacheFeature()
	{
		...
		$sql="SELECT fvl.id_feature_value,fvl.value,fv.id_feature
					...
					WHERE fvl.id_lang=".intval($defaultLanguage->id);
		$res=Db::getInstance()->ExecuteS($sql);
		$featureValues=array();
		foreach($res AS $fv)
		{
