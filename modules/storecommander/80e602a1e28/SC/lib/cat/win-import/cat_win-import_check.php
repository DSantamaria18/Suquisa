...
if(!empty($filename))
{
	...
	if($importConfig[$filename]['forfoundproduct']=="skip" && in_array('attribute',$mappingData['DBArray']))
	...
	if($importConfig[$filename]['fornewproduct']=="create" && $importConfig[$filename]['forfoundproduct']=="create" && in_array('attribute',$mappingData['DBArray']))
	...
	$DATAFILE=file_get_contents(SC_CSV_IMPORT_DIR.$filename);
	$DATA = preg_split("/(?:\r\n|\r|\n)/", $DATAFILE);
	...
	$return .= '<img src="lib/img/accept.png" alt="" style="margin-bottom: -4px;" /> <a href="'.$link.'" target="_blank"><b>'._l("Is your import ready? See the Checklist!").'</b></a>';
}
