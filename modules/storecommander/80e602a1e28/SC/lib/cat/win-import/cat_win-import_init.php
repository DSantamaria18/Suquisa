...
// IMPORT
	...
	arrayFieldOption=Array('feature','feature_custom','attribute','attribute_multiple'<?php if(SCAS) echo ",'quantity','location','add_quantity','remove_quantity'"; if(version_compare(_PS_VERSION_, '1.5.0.0', '>=')) echo ",'supplier_reference','wholesale_price','supplier_currency'" ?>);
	var comboArray = null;
...
//					wImport._uploadWindow = dhxWins.createWindow("wImportUpload", 50, 50, 396, 568);
	...
	wImport.gridFilesDataProcessor.setDataColumns(Array(false,false,false,true,true,true,true,true,true,true,true,true,true,true,true,false));
<?php
	...
	function onClickMapping(id){
									...
									if (wImport.gridMapping.cells(wImport.gridMapping.getRowIndex(id),1).getValue()!='')
										for(var i=0; i < mapping.length; i++)
										{
											...
											if (wImport.gridMapping.cells(wImport.gridMapping.getRowIndex(id),1).getValue()==map[0])
											{
												...
												<?php if(SCAS) { ?>
												if(map[1]=='quantity' || map[1]=='location' || map[1]=='add_quantity' || map[1]=='remove_quantity')
												{
												...
												if(map[1]=='supplier_reference')
												{
													...
													<?php
														...
														$query->leftJoin('supplier_lang', 'sl', 's.`id_supplier` = sl.`id_supplier` AND sl.`id_lang` = '.(int)$sc_agent->id_lang);
								...
								if(!check)
								{
									$.each(optionLabelArray, function(id, label){
										...
										if(name.search(label)>=0)
										{
											finded = true;
											<?php if(SCAS) { ?>
											if(id=='quantity' || id=='location' || id=='add_quantity' || id=='remove_quantity' || id=='supplier_reference' || id=='wholesale_price')
										...
										if(finded)
										{
											...
											{
												<?php if(SCAS) { ?>
												if(id=='quantity' || id=='location' || id=='add_quantity' || id=='remove_quantity')
												{
			...
			if (id=='saveasbtn')
			{
				if (wImport.tbMapping.getValue('saveas')=='')
				{
					...
					$.post('index.php?ajax=1&act=cat_win-import_process&action=mapping_saveas',{'filename':wImport.tbMapping.getValue('saveas'),'mapping':mapping},function(data){
								dhtmlx.message({text:data,type:'info'});
			...
			if (id=='delete')
			{
				if (wImport.tbMapping.getValue('saveas')=='')
				{
					...
					if (confirm('<?php echo _l('Do you want to delete the current mapping?',1)?>'))
						$.get('index.php?ajax=1&act=cat_win-import_process&action=mapping_delete&filename='+wImport.tbMapping.getValue('saveas'),function(data){
...
<?php
	$features=Db::getInstance()->executeS('
							...
							LEFT JOIN `'._DB_PREFIX_.'feature_lang` fl ON (f.`id_feature` = fl.`id_feature` AND fl.`id_lang` = '.(int)$sc_agent->id_lang.')');
...
<?php
	$groups=Db::getInstance()->executeS('
		...
		LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl
			ON (ag.`id_attribute_group` = agl.`id_attribute_group` AND `id_lang` = '.(int)$sc_agent->id_lang.')
		ORDER BY `name` ASC');
		...
		function(id){
			if (id=='check')
			{
				...
				if(strtolower($iso)=="fr")
					$link = 'http://www.storecommander.com/redir.php?dest=2014080702';
				else
					$link = 'http://www.storecommander.com/redir.php?dest=2014080701';
				?>
