	...
	switch($action){
		...
		case 'mapping_process':
			...
			switch ($importConfig[$filename]['idby']){
				case'prodname':
					if (!in_array('name_'.$defaultLanguage->iso_code,$mappingData['CSV2DBOptionsMerged']))
					...
					break;
				case'prodref':
					if (!in_array('reference',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the field reference'));
					break;
				case'prodrefthenprodname':
					if (!in_array('reference',$mappingData['DBArray']) || !in_array('name_'.$defaultLanguage->iso_code,$mappingData['CSV2DBOptionsMerged']))
				...
				case'suprefthenprodname':
					if (!in_array('supplier_reference',$mappingData['DBArray']) || !in_array('name_'.$defaultLanguage->iso_code,$mappingData['CSV2DBOptionsMerged']))
						die(_l('Wrong mapping, mapping should contain the fields supplier_reference and name in ').$defaultLanguage->iso_code);
					break;
				case'supref':
					if (!in_array('supplier_reference',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the field supplier_reference'));
					break;
				case'prodrefandsupref':
					if (!in_array('reference',$mappingData['DBArray']) || !in_array('supplier_reference',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the fields reference and supplier_reference'));
					break;
				case'prodnameandsupref':
					if (!in_array('name_'.$defaultLanguage->iso_code,$mappingData['CSV2DBOptionsMerged']) || !in_array('supplier_reference',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the fields supplier_reference and name in ').$defaultLanguage->iso_code);
					break;
				case'idproduct':
					if (!in_array('id_product',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the field: id_product'));
					break;
				case'idproductattribute':
					if (!in_array('id_product_attribute',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the field id_product_attribute'));
					break;
				case'ean13':
					if (!in_array('EAN13',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the EAN field'));
					break;
				case'upc':
					if (!in_array('upc',$mappingData['DBArray']))
			...
			// get first line
			$DATAFILE=file_get_contents(SC_CSV_IMPORT_DIR.$TODOfilename);
			$DATA = preg_split("/(?:\r\n|\r|\n)/", $DATAFILE);
			...
			if (
					in_array('feature',$mappingData['DBArray'])
					|| in_array('feature_custom',$mappingData['DBArray'])
					|| in_array('feature_add',$mappingData['DBArray'])
					|| in_array('feature_delete',$mappingData['DBArray'])
				)
			{
				refreshCacheFeature();
			}
			...
			if (in_array('id_category_default',$mappingData['DBArray']) || in_array('category_default',$mappingData['DBArray']) || in_array('categories',$mappingData['DBArray']))
			{
...
//if ($kk==374) die('rr');
			...
			// VAT first check
			if ((in_array('priceinctax',$mappingData['DBArray']) || in_array('priceinctaxincecotax',$mappingData['DBArray'])) && !in_array('VAT',$mappingData['DBArray']))
...
//			$dataArray_attributegroupmultiple=array();
			...
			for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA)-1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'],$DATA[$current_line])) && ($current_line <= $importlimit)) ; $current_line++)
			{
				...
				if (in_array('VAT',$mappingData['DBArray']))
				{
				...
				if (in_array('supplier',$mappingData['DBArray']))
				{
					$dataArray_supplier[]=findCSVLineValue('supplier');
				}
				if (in_array('supplier_default',$mappingData['DBArray']))
				{
					$dataArray_supplier[]=findCSVLineValue('supplier_default');
				}
				if (in_array('manufacturer',$mappingData['DBArray']))
				{
					$dataArray_manufacturer[]=findCSVLineValue('manufacturer');
				}
				if (in_array('feature',$mappingData['DBArray']))
				{
					findAllCSVLineValue('feature',$dataArray_feature,'id_feature',$features);
				}
				if (in_array('feature_add',$mappingData['DBArray']))
				{
...
echo "</pre>";*/
			...
			{
				if ( in_array($mappingData['CSV2DB'][$CSVFieldName], array('feature','feature_custom','feature_add','feature_delete')))
				{
					if (!in_array($featureName,array_keys($features)) && !in_array($featureName,$err_unique))
					{
			...
			if (!$isErr)
			{
				...
				foreach($dataArray_feature AS $f)
				{
					...
					if ($f['value']!='' && !in_array($f['id_feature'].'_|_'.$f['value'],array_keys($featureValues)) && !in_array($f['id_feature'].'x|x'.$f['object'].'x|x'.$f['id_feature'].'_|_'.$f['value'],$dataArray_feature_unique))
					{
			...
			foreach($mappingData['CSV2DBOptions'] AS $CSVFieldName => $attributeGroupName)
			{
				...
				{
					if (!in_array($attributeGroupName,array_keys($attributeGroups)) && !in_array($attributeGroupName,$err_unique))
					{
...
//!in_array($ag['value'],array_keys($attributeValues),true)
			...
			$refsupp_option = "";
			if(in_array($importConfig[$TODOfilename]['idby'], array('supref')))
			{
			...
			$CSVDataStr = file_get_contents(SC_CSV_IMPORT_DIR.$TODOfilename);
			$CSVData = preg_split("/(?:\r\n|\r|\n)/", $CSVDataStr);
			$lastIdentifier='';
			$lastid_product=0;
...
//var_dump($aa);
			for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA)-1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'],$DATA[$current_line])) && ($current_line <= $importlimit)) ; $current_line++)
			{
				...
				if(!empty($id_product))
				{
					...
					elseif($importConfig[$TODOfilename]['forfoundproduct']=='update')
					{
						...
						elseif(version_compare(_PS_VERSION_, '1.5.0.0', '>='))
							$newprod=new Product($id_product, false, null,1);
						else
					...
					elseif($importConfig[$TODOfilename]['forfoundproduct']=='create')
					{
						...
						)
						{
							...
							elseif(version_compare(_PS_VERSION_, '1.5.0.0', '>='))
								$newprod=new Product($lastid_product, false, null,1);
							else
				...
				foreach($line AS $key => $value)
				{
					...
					if (sc_array_key_exists($key,$firstLineData) && in_array($firstLineData[$key],$mappingData['CSVArray']))
					{
						if ($scdebug) echo 'c';
...
/*								if ($importConfig[$TODOfilename]['iffoundindb']=='replace'
							...
							case 'VAT':
								...
								if (version_compare(_PS_VERSION_, '1.4.0.0', '<'))
								{
									...
									$newprod->id_tax = intval(Tax::getTaxIdByRate($newprod->tax_rate));
								}else{
									$newprod->tax_rate=importConv2Float($value);
									$vat_string=trim( trim( trim ( importConv2Float($value) , '0' )) , '.');
									$sql="SELECT trg.name, trg.id_tax_rules_group
									FROM `"._DB_PREFIX_."tax_rules_group` trg
									LEFT JOIN `"._DB_PREFIX_."tax_rule` tr ON (trg.`id_tax_rules_group` = tr.`id_tax_rules_group` AND tr.`id_country` = ".(int)SCI::getDefaultCountryId()." AND tr.`id_state` = 0)
							 	  LEFT JOIN `"._DB_PREFIX_."tax` t ON (t.`id_tax` = tr.`id_tax`)
							    WHERE trg.active=1 AND (trg.name LIKE '%".psql($vat_string)."%' OR trg.name LIKE '%".psql(str_replace(',','.',$vat_string))."%')";
									$DB_VAT=Db::getInstance()->getRow($sql);
									$newprod->id_tax_rules_group = intval($DB_VAT['id_tax_rules_group']);
								}
								break;
							case 'priceexctax':
								if (isCombination($line) || isCombinationWithID())
								...
								break;
							case 'priceinctax':
// TODO : besoin d'optimiser le calcul pour ne le faire qu'une fois par ligne csv
								if (version_compare(_PS_VERSION_, '1.4.0.0', '<'))
								{
									$rate=1+(importConv2Float(findCSVLineValue('VAT'))/100);
								}else{
									$vat_string=trim( trim( trim ( importConv2Float( findCSVLineValue('VAT') ) , '0' )) , '.');
									$sql="SELECT t.rate
									FROM `"._DB_PREFIX_."tax_rules_group` trg
									LEFT JOIN `"._DB_PREFIX_."tax_rule` tr ON (trg.`id_tax_rules_group` = tr.`id_tax_rules_group` AND tr.`id_country` = ".(int)SCI::getDefaultCountryId()." AND tr.`id_state` = 0)
							 	  LEFT JOIN `"._DB_PREFIX_."tax` t ON (t.`id_tax` = tr.`id_tax`)
							    WHERE trg.active=1 AND (trg.name LIKE '%".psql($vat_string)."%' OR trg.name LIKE '%".psql(str_replace(',','.',$vat_string))."%')";
									$DB_VAT=Db::getInstance()->getRow($sql);
									$rate=1+(floatval($DB_VAT['rate'])/100);
								}
								if (isCombination($line) || isCombinationWithID())
								{
									// set price for first combination)
...
// TODO : besoin d'optimiser le calcul pour ne le faire qu'une fois par ligne csv
								if (version_compare(_PS_VERSION_, '1.4.0.0', '<'))
								{
									$rate=1+( trim( trim ( importConv2Float(findCSVLineValue('VAT')) , '0' )) /100);
								}else{
									$vat_string=trim( trim( trim ( importConv2Float( findCSVLineValue('VAT') ) , '0' )) , '.');
									$sql="SELECT t.rate
									FROM `"._DB_PREFIX_."tax_rules_group` trg
							...
							case 'name':
								...
								if (!in_array('link_rewrite',$mappingData['DBArray']))
								{
									foreach($languages AS $lang)
									{
							...
							case 'categories':
								...
								if ($value!='')
								{
									...
									foreach($categ AS $c)
									{
										$c=cleanQuotes($c);
										$idc=0;
										if ((strpos($c,'>')!==false) || (in_array(trim($value),$categoriesFirstLevel))) // category path
										{
							...
							case 'attachments':
								...
								{
									$TODO[] = "DELETE FROM `"._DB_PREFIX_."product_attachment` WHERE `id_attachment` IN (".join(',',$attachmentsID).") AND `id_product`=ID_PRODUCT";
									foreach($attachmentsID AS $aid)
										$TODO[] = "INSERT INTO `"._DB_PREFIX_."product_attachment` (id_product,id_attachment) VALUES (ID_PRODUCT,".intval($aid).")";
									if (version_compare(_PS_VERSION_,'1.4.0.2','>='))
										$TODO[]="UPDATE `"._DB_PREFIX_."product` SET cache_has_attachments=1 WHERE `id_product`=ID_PRODUCT";
								}
								break;
...
/* inutile pour le moment, on gère champ par champ
				...
				if (_s('APP_DEBUG_CATALOG_IMPORT'))
				{
					$time_end = microtime(true);
					$time = $time_end - $time_start;
					echo "<br/><br/>Before product->save : $time seconds";
				}
				foreach($languages AS $lang)
				...
				if ($newprod->save())
				{
					...
					if (_s('APP_DEBUG_CATALOG_IMPORT'))
					{
						$time_end = microtime(true);
						$time = $time_end - $time_start;
						echo "<br/><br/>After updateProduct Hook : $time seconds";
					}
					if ($scdebug) echo 'e';
					...
					if (_s('APP_DEBUG_CATALOG_IMPORT'))
					{
						$time_end = microtime(true);
						$time = $time_end - $time_start;
						echo "<br/><br/>Before product TODO  : $time seconds";
					}
					...
					if (_s('APP_DEBUG_CATALOG_IMPORT'))
					{
						$time_end = microtime(true);
						$time = $time_end - $time_start;
						echo "<br/><br/>After product TODO : $time seconds";
					}
					...
					if(!empty($newprod->id))
					{
						...
						elseif(version_compare(_PS_VERSION_, '1.5.0.0', '>='))
							$newprod=new Product($newprod->id, false, null,1);
						else
							...
							switch($switchObject)
							{
								...
								case 'priceexctax':
									...
									if (isCombinationWithID())
									{
										...
										if (version_compare(_PS_VERSION_, '1.4.0.4', '<'))
										{
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET price=".importConv2Float((importConv2Float($value)*$rate)-($newprod->price*$rate))." WHERE id_product_attribute=".intval($id_product_attribute);
										}else{
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET price=".importConv2Float(importConv2Float($value)-$newprod->price)." WHERE id_product_attribute=".intval($id_product_attribute);
										}
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET price=".importConv2Float(importConv2Float($value)-$newprod->price)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'price_impact':
									if (isCombination($line))
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET price=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET price=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'sc_active':
									if (isCombination($line))
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET sc_active=".intval($value)." WHERE id_product_attribute=".intval($id_product_attribute);
									}
									break;
									...
									if (isCombinationWithID() && !empty($id_product_attribute) && getBoolean($value))
									{
										if (version_compare(_PS_VERSION_,'1.5.0.0','>='))
										{
											if($value) {
												$TODO[] = "UPDATE " . _DB_PREFIX_ . "product_attribute_shop SET default_on=0 WHERE id_shop IN (" . join(',',
														$id_shop_list) . ")";
												$TODO[] = "UPDATE " . _DB_PREFIX_ . "product_attribute_shop SET default_on=1 WHERE id_product_attribute=" . intval($id_product_attribute) . " AND id_shop IN (" . join(',',
														$id_shop_list) . ")";
										...
										}elseif (version_compare(_PS_VERSION_,'1.4.0.0','>='))
										{
											...
											if($value)
											{
												...
												$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET default_on=1 WHERE id_product_attribute=".intval($id_product_attribute);
											}
										}
									}
									break;
...
// TODO : besoin d'optimiser le calcul pour ne le faire qu'une fois par ligne csv
										...
										if (version_compare(_PS_VERSION_, '1.4.0.4', '<'))
										{
											$combinationValues['price']=(importConv2Float(importConv2Float($value)/$rate)-$newprod->price)*$rate;
										}else{
...
// TODO : besoin d'optimiser le calcul pour ne le faire qu'une fois par ligne csv
										...
										if (version_compare(_PS_VERSION_, '1.4.0.4', '<'))
										{
											...
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET price=".(importConv2Float(importConv2Float($value)/$rate)-$newprod->price)." WHERE id_product_attribute=".intval($id_product_attribute);
										}
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET price=".(importConv2Float(importConv2Float($value)/$rate)-$newprod->price)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'unit_price_impact':
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET unit_price_impact=".floatval(importConv2Float($value))." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET unit_price_impact=".floatval(importConv2Float($value))." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'weight':
									if (isCombination($line))
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET weight=".floatval(importConv2Float($value)-$newprod->weight)." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET weight=".floatval(importConv2Float($value)-$newprod->weight)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'weight_impact':
									if (isCombination($line))
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET weight=".floatval(importConv2Float($value))." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET weight=".floatval(importConv2Float($value))." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'EAN13':
									if (isCombination($line))
									...
									if (isCombinationWithID())
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET ean13='".psql($value)."' WHERE id_product_attribute=".intval($id_product_attribute);
									break;
								case 'upc':
									if (isCombination($line))
										$combinationValues['upc']=$value;
									if (isCombinationWithID())
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET upc='".psql($value)."' WHERE id_product_attribute=".intval($id_product_attribute);
									break;
								case 'advanced_stock_management':
									if (/*!isCombination($line) &&*/ SCAS)
									{
										$type = intval($value);
										if(!in_array($type, array(1,2,3)) && empty($id_product))// si 0 et création
											$type = _s("CAT_ADVANCEDSTOCK_DEFAULT");
								...
								case 'wholesale_price':
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET wholesale_price=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET wholesale_price=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
										...
										elseif( version_compare(_PS_VERSION_, '1.5.0.0', '>=') && !empty($id_supplier) && $id_supplier!="product")
										{
											...
											if(!empty($id_supplier))
											{
												...
												if($id_supplier==$newprod->id_supplier)
													$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET product_attribute_shop='".psql(importConv2Float($value))."' WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
											}
										}
									}
								...
								case 'ecotax':
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET ecotax=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET ecotax=".importConv2Float($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'date_available':
									if (isCombination($line))
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET available_date='".pSQL($value)."' WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET available_date='".pSQL($value)."' WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'minimal_quantity':
									...
									if (isCombinationWithID())
									{
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET minimal_quantity=".intval($value)." WHERE id_product_attribute=".intval($id_product_attribute);
										if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
											$TODO[]="UPDATE "._DB_PREFIX_."product_attribute_shop SET minimal_quantity=".intval($value)." WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")";
									}
									break;
								case 'quantity'://$mappingData['CSV2DBOptions'][$firstLineData[$key]]
													...
													if(!empty($id_warehouse) && is_numeric($id_warehouse) && $id_warehouse>0 &&  $value>=0)
													{
														...
														if($value>=0) // add stock
														{
															...
															try {
																...
																{
																	$stock_manager->removeProduct($newprod->id, 0, $warehouse, $avanced_quantities["physical_quantity"], 4, 1);
																}
															} catch (Exception $e) {
															...
															if($value>0)
															{
																...
																if ($stock_manager->addProduct((int)$newprod->id, 0, $warehouse, (int)$value, 1, floatval($price), 1))
																{
														...
														/*elseif(!empty($value) && $value<0) // delete stock
														{
															if(empty($warehousesArray[$id_warehouse]))
															{
																...
																$warehouse = $warehousesArray[$id_warehouse];
						          							$value = $value * -1;
															$removed_products = $stock_manager->removeProduct((int)$newprod->id, 0, $warehouse, $value, 2, 1);
									...
									if (isCombinationWithID() && version_compare(_PS_VERSION_, '1.5.0.0', '<'))
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET quantity=".intval($value)." WHERE id_product_attribute=".intval($id_product_attribute);
									break;
								case 'add_quantity':
									$value = importConv2Int($value);
									if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
										...
										if (!empty($newprod->id) && !isCombination($line) && empty($id_product_attribute) && !$newprod->hasAttributes())
										{
											...
											if($no_scas && ( !SCAS || (SCAS && empty($id_warehouse) ) ) ) // Si désactivé et que option non entrepot
											{
												if(empty($newprod->id))
												{
													foreach($id_shop_list AS $id_shop_temp)
													{
														...
														if ($id_stock_available)
															StockAvailable::updateQuantity((int)$newprod->id, 0, (int)$value, (int)$id_shop_temp);
														else
															SCI::setQuantity((int)$newprod->id, 0, (int)$value, $id_shop_temp);
													}
												}
												else
												{
													...
													if ($id_stock_available)
														StockAvailable::updateQuantity((int)$newprod->id, 0, (int)$value, (int)$id_shop);
													else
														SCI::setQuantity((int)$newprod->id, 0, (int)$value, $id_shop);
												}
											}
									...
									if (isCombinationWithID() && version_compare(_PS_VERSION_, '1.5.0.0', '<'))
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET quantity=quantity+".intval($value)." WHERE id_product_attribute=".intval($id_product_attribute);
									break;
								case 'remove_quantity':
									$value = importConv2Int($value);
									if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
									{
										...
										if (!empty($newprod->id) && !isCombination($line) && empty($id_product_attribute) && !$newprod->hasAttributes())
										{
											...
											if($no_scas && ( !SCAS || (SCAS && empty($id_warehouse) ) ) ) // Si désactivé et que option non entrepot
											{
												...
												{
													foreach($id_shop_list AS $id_shop_temp)
														StockAvailable::updateQuantity((int)$newprod->id, 0, (int)($value*-1), $id_shop_temp);
												}
												else
													StockAvailable::updateQuantity((int)$newprod->id, 0, (int)($value*-1), $id_shop);
											}
										}
									}
									elseif (isCombination($line) && !isCombinationWithID())// si nouvelle déclinaison, on met directement dans quantité
								...
								case 'reference':
									...
									if (isCombinationWithID())
										$TODO[]="UPDATE "._DB_PREFIX_."product_attribute SET reference='".psql($value)."' WHERE id_product_attribute=".intval($id_product_attribute);
									break;
								case 'supplier_reference':
									if (isCombination($line))
								...
								case 'ActionDeletesWarehouses':
									if ($value!='' && $value!='0' && $newprod->id)
									{
										$sql = "DELETE FROM `"._DB_PREFIX_."warehouse_product_location` WHERE id_product = '".intval($newprod->id)."' ".(!empty($id_product_attribute)?" id_product_attribute = '".intval($id_product_attribute)."' ":'');
										Db::getInstance()->execute($sql);
									}
									break;
								case 'tags':
									if (count($productsWithTagUpdatedList)==0 || (!sc_array_key_exists($id_lang,$productsWithTagUpdatedList) || !in_array($newprod->id,$productsWithTagUpdatedList[$id_lang])))
									{
										$value=trim(trim($value),$importConfig[$TODOfilename]['valuesep']);
										if ($value!='')
										{
								...
								case 'ActionDeleteAllCombinations':
									if (getBoolean($value) && $newprod->id)
									{
								...
								case 'customization_field_type':
									...
									if ($newprod->id)
									{
										if (!in_array($cfrequired,array(0,1))) $cfrequired=0;
										if (!in_array($value,array(0,1))) $value=0;
          		...
          		if (_s('APP_DEBUG_CATALOG_IMPORT'))
          		{
          			$time_end = microtime(true);
          			$time = $time_end - $time_start;
          			echo "<br/><br/>Fields processins after product creation : $time seconds";
          		}
          		if(!empty($image_id_array))
          		{
          			$TODO_image = array();
          			foreach($image_id_array as $actual_num=>$image_id)
          			{
          				...
          				if(!empty($image_legend_array))
          				{
          					$actual_legend = $image_legend_array[$actual_num]["value"];
          					$actual_legend_lang = intval($getIDlangByISO[$image_legend_array[$actual_num]["option"]]);

          					if(!empty($actual_legend_lang))
          					{
          						$sql="SELECT * FROM "._DB_PREFIX_."image_lang WHERE id_image='".intval($actual_id)."' AND id_lang = ".intval($actual_legend_lang);
          						$img_row = Db::getInstance()->ExecuteS($sql);
          						if(!empty($img_row[0]["id_image"]))
          						{
...
//								 $default, $location = null, $upc = null, $minimal_quantity = 1, array $id_shop_list = array())
								$id_product_attribute = $newprod->addAttribute(
										round(importConv2Float($v['price']),6),
										$v['weight'],
										0, // unit_impact
...
//						$newprod->deleteDefaultAttributes();
          ...
          if (version_compare(_PS_VERSION_,'1.5.0.0','>=') && !empty($id_product_attribute))
          {
	          					...
	          					if( version_compare(_PS_VERSION_, '1.5.0.0', '>=') && !empty($id_supplier) && $id_supplier!="product")
	          					{
	          						...
	          						if(!empty($id_supplier))
	          						{
	          							foreach($combinations_ids as $id_product_attribute)
	          							{
		          							...
		          							$product_supplier->save();
		          							if($id_supplier==$newprod->id_supplier)
		          								Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."product_attribute_shop SET product_attribute_shop='".psql(importConv2Float($value))."' WHERE id_product_attribute=".intval($id_product_attribute)." AND id_shop IN (".join(',',$id_shop_list).")");
	          							}
	          						}
	          					}
						          		...
						          		if(!empty($id_warehouse) && is_numeric($id_warehouse) && $id_warehouse>0 && $value>=0) // Si option avec entrepot
						          		{
				          					foreach($combinations_ids as $id_product_attribute)
				          					{
							          			...
							          			if($value>=0) // add stock
							          			{
													...
													if(!empty($avanced_quantities["physical_quantity"]))
													{
														$stock_manager->removeProduct((int)$newprod->id, (int)$id_product_attribute, $warehouse, $avanced_quantities["physical_quantity"], 4, 1);
													}
													...
													if($value>0)
													{
														// ADD STOCK FOR COMBINATION
								          				...
								          				if ($stock_manager->addProduct((int)$newprod->id, (int)$id_product_attribute, $warehouse, (int)$value, 1, floatval($price), 1))
								          				{
								          					StockAvailable::synchronize((int)$newprod->id);
								          				}
								          				else
							          			...
							          			/*elseif(!empty($value) && $value<0) // delete stock
							          			{
							          				if(empty($warehousesArray[$id_warehouse]))
													{
														...
														$warehouse = $warehousesArray[$id_warehouse];
							          				$value = $value * -1;
							          				$removed_products = $stock_manager->removeProduct((int)$newprod->id, (int)$id_product_attribute, $warehouse, $value, 2, 1);
	         					...
	         					if(SCAS)
	         					{
					          	...
					          	if($no_scas && ( !SCAS || (SCAS && empty($id_warehouse) ) ) ) // Si désactivé et que option non entrepot
					          	{
		          					foreach($combinations_ids as $id_product_attribute)
		          					{
										...
										if ($id_stock_available)
					          				StockAvailable::updateQuantity((int)$newprod->id, (int)$id_product_attribute, (int)$value, (int)$id_shop);
										else
					          				SCI::setQuantity((int)$newprod->id, (int)$id_product_attribute, (int)$value, $id_shop);
									}
					          	}
	          				...
	          				case 'remove_quantity':
	         					...
	         					if(SCAS)
	         					{
					          	...
					          	if($no_scas && ( !SCAS || (SCAS && empty($id_warehouse) ) ) ) // Si désactivé et que option non entrepot
					          	{
		          					foreach($combinations_ids as $id_product_attribute)
		          					{
						          		StockAvailable::updateQuantity((int)$newprod->id, (int)$id_product_attribute, (int)($value*-1), (int)$id_shop);
		          					}
					          	}
							break;
							default:
          	...
          	if (_s('APP_DEBUG_CATALOG_IMPORT'))
          	{
          		$time_end = microtime(true);
          		$time = $time_end - $time_start;
          		echo "<br/><br/>Combinations processing : $time seconds";
          	}

					// link images to combinations
         	if (((isCombination() && $combinationUsed) || isCombinationWithID()) && _s('CAT_PROD_IMG_SAVE_FILENAME'))
         	{
         		if (isCombinationWithID())
         		{
         			...
         			foreach($res AS $v)
         			{
						...
						{
							$sql='';
							if (in_array('supplier_reference',$mappingData['DBArray']) && findCSVLineValue('supplier_reference')!='')
							...
							if (version_compare(_PS_VERSION_,'1.5.0.0','>=') && in_array('supplier_reference',$mappingData['DBArray']) && findCSVLineValue('supplier_reference')!='')
							...
							if (in_array('reference',$mappingData['DBArray']) && findCSVLineValue('reference')!='')
         	...
         	if ($id_product_attribute!=0)
         	{
					...
					{
						$value = trim($value);
						if (sc_array_key_exists($key, $firstLineData) && in_array($firstLineData[$key],
								$mappingData['CSVArray'])
						) {
			...
			if ((count($CSVData)==1) || (count($CSVData)==2 && $CSVData[0]==join('',$CSVData)) || (filesize(SC_CSV_IMPORT_DIR.$TODOfilename)==0))
			{
				...
				if (in_array('priceinctax',$mappingData['DBArray']))
				{
