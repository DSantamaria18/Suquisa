...
function getXml($id_category)
{
	...
	if(!empty($id_category))
	{
		...
		// FILTERS
			...
			if(!in_array("dat_unlimited", $filters)!==false && !empty($filters) && !empty($filters[0]))
			{
				$where .= ' AND ( `to`!="0000-00-00 00:00:00" ) ';
			}
			elseif(in_array("dat_unlimited", $filters)!==false && count($filters)==1)
			{
				$where .= ' AND ( `to`="0000-00-00 00:00:00" ) ';
			}
