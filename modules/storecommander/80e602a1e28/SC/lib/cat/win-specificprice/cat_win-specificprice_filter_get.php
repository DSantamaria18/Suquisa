...
$icon='catalog.png';
	...
	echo '<item id="Gdat" text="'._l('Date').'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" open="1">'."\n";
		echo '<item id="dat_past" text="'._l('Before').' '.$dateT.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array("dat_past",$selection)?'checked="1"':'').'></item>'."\n";
		echo '<item id="dat_present" text="'._l('On').' '.$dateT.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array("dat_present",$selection)?'checked="1"':'').'></item>'."\n";
		echo '<item id="dat_futur" text="'._l('After').' '.$dateT.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array("dat_futur",$selection)?'checked="1"':'').'></item>'."\n";
		echo '<item id="dat_unlimited" text="'._l('Unlimited').'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array("dat_unlimited",$selection)?'checked="1"':'').'></item>'."\n";
	echo '</item>'."\n";
	...
	echo '<item id="Gsup" text="'._l('Suppliers').'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'">'."\n";
		...
		foreach($suppliers as $supplier)
		{
			...
			echo '<item id="sup_'.$supplier['id_supplier'].'" text="'.$name.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array('sup_'.$supplier['id_supplier'],$selection)?'checked="1"':'').'></item>'."\n";
		}
	echo '</item>'."\n";
	...
	*/
	...
	foreach($manufacturers as $manufacturer)
	{
		...
		$name = str_replace(">", "2", $name);
		$name = str_replace('"', "''", $name);
		echo '<item id="man_'.$manufacturer['id_manufacturer'].'" text="'.$name.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array('man_'.$supplier['id_manufacturer'],$selection)?'checked="1"':'').'>'."\n";
		echo '</item>'."\n";
	}
	echo '</item>'."\n";
	...
	*/
	...
	if(!empty($countries) && count($countries))
	{
		...
		foreach($countries as $country)
		{
			...
			$name = str_replace(">", "2", $name);
			$name = str_replace('"', "''", $name);
			echo '<item id="cou_'.$country['id_country'].'" text="'.$name.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(in_array('cou_'.$country['id_country'],$selection)?'checked="1"':'').'>'."\n";
			echo '</item>'."\n";
		}
		echo '</item>'."\n";
	}
