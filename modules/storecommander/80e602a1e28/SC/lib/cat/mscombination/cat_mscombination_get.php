	...
	function generateValue($col, $product_attribute, $product, $taxrate)
	{
		...
		switch($col){
			case'ecotax':
				...
				$return .= "<cell>".number_format(  $ecotax , 2, '.', '')."</cell>";
				break;
			case'taxrate':
			...
			case'price':
				...
				if(_s('CAT_PROD_ECOTAXINCLUDED'))
					$ecotax = $product_attribute['ecotax'] * SCI::getEcotaxTaxRate();
				$return.=("<cell>".number_format($product_attribute['price']*($taxrate/100+1)+$product['price']*($taxrate/100+1) + $ecotax, 2, '.', '')."</cell>");
				break;
			case'pprice':
				$ecotax = 0;
				if(_s('CAT_PROD_ECOTAXINCLUDED'))
			...
			case'ppriceextax':
				...
				break;
			case'priceextax':
				$return.=("<cell>".number_format($product_attribute['price']+$product['price'], 2, '.', '')."</cell>");
				break;
			case'margin':
				$return.="<cell></cell>";
				break;
			...
			case'quantity':
				...
				$return .= "<cell>".SCI::getProductQty((int)$product['id'], (int)$product_attribute['id'])."</cell>";
				break;
			...
			default:
				if (sc_array_key_exists('buildDefaultValue',$colSettings[$col]) && $colSettings[$col]['buildDefaultValue']!='')
				{
					...
					elseif ($col=='id_product_attribute')
						$return .= "<cell>".$product_attribute['id']."</cell>";
					elseif ($col=='id_shop')
