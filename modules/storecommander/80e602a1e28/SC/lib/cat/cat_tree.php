...
?>
	...
	function onClickShopTree(idshop, param,callback){
		...
		if (idshop != shopselection)
		{
			...
			//cat_tb.callEvent("onClick",['refresh']);
			var callback_refresh = "";
			/*if(idshop==0)
				callback_refresh = "cat_tree.enableDragAndDrop('temporary_disabled')";
			else*/
				callback_refresh = "cat_tree.enableDragAndDrop(true);";
...
?>
	...
	//cat_tree.enableDragAndDropScrolling(true);
<?php if (!SCMS){ ?>
		displayTree();
<?php } ?>
...
?>
		...
		if (itemId=="sort"){
			tabId=cat_tree.contextID;
			cat_tree.enableDragAndDrop('temporary_disabled');
			cat_tree.setCustomSortFunction(nameSort);
