...
<?php } ?>
			...
			function onCheckImage(rId,cInd,state)
			{
				...
				if (cId!='cover')
				{
					...
					$.post("index.php?ajax=1&act=cat_image_update&action=shop&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'id_product': lastProductSelID, 'list_id_image':rId,shop:shop,val:Number(state)},function(data){});
				}else{
...
?>
		...
		if (id=='image_fill_legend')
		{
			if (prop_tb._imagesGrid.getSelectedRowId()==null)
			{
				...
				if (lastProductSelID!=0)
				{
					if (confirm('<?php echo _l('Are you sure you want to change the legend of the selected image(s) with the product name?',1)?>'))
					{
						list=prop_tb._imagesGrid.getSelectedRowId().split(',');
						idxLegend=prop_tb._imagesGrid.getColIndexById('legend');
						idxProductName=cat_grid.getColIndexById('name');
						$.post("index.php?ajax=1&act=cat_image_update&action=update&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{'col':'legend','val':cat_grid.cells(lastProductSelID,idxProductName).getValue().replace(/#/g,''),'id_product': lastProductSelID, 'list_id_image':prop_tb._imagesGrid.getSelectedRowId()},function(){});
