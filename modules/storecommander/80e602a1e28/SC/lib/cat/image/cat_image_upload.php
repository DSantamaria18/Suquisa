		...
		var uploader = $("#uploader").pluploadQueue({
			...
			init : {
	            BeforeUpload : function(up){
					$("#uploader").pluploadQueue().settings.url='index.php?ajax=1&act=all_upload&obj=image&product_list=<?php echo $product_list;?>&attr_list=<?php echo $attr_list;?>&id_lang=<?php echo $id_lang;?>';
	            },
	            UploadComplete : function(up){
	            	if (noerror)
						top.prop_tb._imagesUploadWindow['<?php echo $product_list; ?>'].hide();
					//if (top.lastProductSelID==<?php echo (int)$product_list;?>)
					<?php if(!$is_attr) { ?>
					top.displayImages();
					<?php } else { ?>
