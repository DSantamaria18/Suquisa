...
//				window.open("index.php?ajax=1&act=all_get-file&path=../import/&file="+wExport.gridFiles.cells(wExport.gridFiles.getSelectedRowId(),idxFilename).getValue());
	...
	dhxlExport.cells('b').setText("<?php echo _l('Mapping').' '._l('Export')?>");
	dhxlExport.cells('b').setWidth(600);
	...
	function onClickMappingExport(id){
			...
			if (id=='saveasbtn')
			{
				...
				else if (checkOptions()==false)
				{
					...
					$.post('index.php?ajax=1&act=cat_win-export_process&action=mapping_saveas',{'filename':wExport.tbMapping.getValue('saveas'),'mapping':mapping},function(data){
			...
			if (id=='delete')
			{
				if (wExport.tbMapping.getValue('saveas')=='')
				{
					...
					if (confirm('<?php echo _l('Delete mapping and reset grid',1)._l('?')?>'))
						$.get('index.php?ajax=1&act=cat_win-export_process&action=mapping_delete&filename='+wExport.tbMapping.getValue('saveas'),function(data){
							wExport.gridMapping.clearAll();
...
//					if (categselection=='') return;
	...
	export_cell.cells('a').attachHTMLString('<div id="export_contener" style="height: 100%; overflow: auto;"><img src="lib/img/loading.gif" alt="loading" title="loading" style="float: left; margin:10px; display: none;" /><div id="export_message" style="padding-left: 10px;font-family: Tahoma; font-size: 11px !important; line-height: 18px;"></div></div>');
	...
	export_cell_files_tbOptions.addButton("refresh", 0, "", "lib/img/arrow_refresh.png", "lib/img/arrow_refresh.png");
	export_cell_files_tbOptions.setItemToolTip('refresh','<?php echo _l('Refresh',1)?>');
	export_cell_files_tbOptions.attachEvent("onClick", function(id){
		if (id=='refresh')
		{
			export_cell.cells('b').attachURL("index.php?ajax=1&act=cat_win-export_files_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(data){});
		}
	});
...
function loadAllFieldsInMapping()
{
	var ii=0;
	...
	for(var j=1;j<nb_max;j++)
	{
		...
		if(flag_lang)
		{
			...
			{ 
				if($num==0) {
					echo "wExport.gridMapping.cells(rId,idxLang).setValue('".$lang['iso_code']."');";
				} else { ?>
					var newId = (new Date()).valueOf()+ii;
