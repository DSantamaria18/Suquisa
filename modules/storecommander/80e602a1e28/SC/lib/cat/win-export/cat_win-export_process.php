...
//			$debug .= Db::getInstance()->getMsgError();
			...
			// supplier
			if (in_array('supplier_reference',$fieldList) || in_array('wholesale_price',$fieldList))
			{
			...
			foreach($products AS $idp)
			{
				...
				// features
				if (in_array('feature',$fieldList) || in_array('multiple_features',$fieldList))
				{
				...
				{		
					...
					)
					{
						...
						foreach($mapping AS $n => $f)
						{
							...
							if (in_array($f['name'],$fields_lang) && ($f['lang']=='' || !sc_array_key_exists($f['lang'],$getIDlangByISO)))
										...
										switch($f['name'])
										{
											...
											case 'category_full_path':
												...
												if ($row = Db::getInstance()->ExecuteS('
													...
													foreach ($row as $val)
														$field .= getCategoryPath($val['id_category'],'',$val['id_category'],$getIDlangByISO[$f['lang']]).$exportConfig['valuesep'];
											...
											case 'priceinctaxwithshipping':
												...
												$weight=$p->weight;
												$field=ps_round(getOrderShippingCost(NULL, true, 0, $weight, $price)+$price,6);
												if ($exportConfig['shippingfee']>0 && $field<=$exportConfig['shippingfeefreefrom'])
													$field+=$exportConfig['shippingfee'];
												$field = ps_round($field,(int)_s('CAT_EXPORT_PRICE_DECIMAL'));
												break;
											...
											case 'available_later':
												if(!empty($id_product_attribute) && SCI::getConfigurationValue("SC_DELIVERYDATE_INSTALLED")=="1")
												{
													...
													$field=$p->available_later[$getIDlangByISO[$f['lang']]];
												break;
											case 'reduction_price':case 'reduction_percent':case 'reduction_from':case 'reduction_to':
												if (version_compare(_PS_VERSION_, '1.4.0.0', '>='))
												{
							...
							if ($f['modifications']!='')
							{
								...
								foreach($tasks AS $t)
									if ($t!='' && (in_array($t,array('strip_tags','strtolower','strtoupper', 'html_entity_decode')) || substr($t,0,1)=='='))
