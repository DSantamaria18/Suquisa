		...
		if (needInitPdtSeo)
		{
			...
			prop_tb._pdtSeoGrid.attachEvent("onBeforeContextMenu", function(rowid,colidx,grid){
				var disableOnCols=new Array(
						...
						prop_tb._pdtSeoGrid.getColIndexById('shop'),
				...
				pdtSeo_cmenu.setItemText('shop', '<?php echo _l('Shop:')?> '+prop_tb._pdtSeoGrid.cells(rowid,prop_tb._pdtSeoGrid.getColIndexById('shop')).getTitle());
				pdtSeo_cmenu.setItemText('shop', '<?php echo _l('Lang:')?> '+prop_tb._pdtSeoGrid.cells(rowid,prop_tb._pdtSeoGrid.getColIndexById('lang')).getTitle());
