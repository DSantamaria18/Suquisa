...
?>
					if (cInd == idxUsed){
						if(stage==0 && prop_tb._categoriesGrid.cells(rId,idxUsed).getValue()==1 && prop_tb._categoriesGrid.cells(rId,idxDefault).getValue()==1)
							return false;
						if(stage==1 && !(prop_tb._categoriesGrid.cells(rId,idxUsed).getValue()==0 && prop_tb._categoriesGrid.cells(rId,idxDefault).getValue()==1))
						{
							var selection = cat_grid.getSelectedRowId();
							ids=selection.split(',');
							$.each(ids, function(num, pId){
				...
				if (data!='')
				{
					...
					prop_tb._categoriesGrid.forEachRow(function(id){
							if (in_array(id,selArray))
							{
								...
								if (prop_tb._categoriesGrid.doesRowExist(id) && (cache_categorypanel_treeticks[id] == 1))
									prop_tb._categoriesGrid.cellById(id,1).setValue(0);
								cache_categorypanel_treeticks[id] = 0;
							}
						});
					if (dataArray[1]!=undefined && dataArray[1]!='' && dataArray[1]>0)
					{
