 ...
 **/
	...
	foreach($rows AS $row)
	{
		...
		$xml.=("<row id='".$row['id_attribute']."'>");
			...
			if ($iscolor) $xml.=(file_exists(_PS_COL_IMG_DIR_.$row['id_attribute'].'.jpg')?'<cell><![CDATA[<img src='._THEME_COL_DIR_.$row['id_attribute'].'.jpg height=40/>]]></cell>':'<cell></cell>');
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[#text_filter<?php echo (version_compare(_PS_VERSION_, '1.5.0.0', '>=')?',#numeric_filter':''); if ($iscolor) {echo ',#text_filter,';}  echo $filters ?>,#numeric_filter]]></param></call>
