...
/*
	...
	*/
	...
	{
		global $sql,$col,$tax,$arrManufacturers,$arrSuppliers,$prodWithAttributes,$countAttributes,$id_lang,$cols,$view,$colSettings,$user_lang_iso,$fields,$fields_lang,$forceUpdateCombinations,$fieldsWithHTML,$prodrow,$exportedProducts;
		$link=new Link();
		$col = "";
		...
		$fields=array('reference','wholesale_price','price','unity','unit_price_ratio','ecotax','weight','supplier_reference','id_manufacturer','id_supplier',
									...
									'available_for_order','show_price','online_only','condition','additional_shipping_cost','available_date','visibility');
			...
			foreach($cols AS $key => $col)
			{
				switch($col){
					...
					case'supplier_reference':
						if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
						{
							$resRefSup=Db::getInstance()->ExecuteS("SELECT product_supplier_reference FROM "._DB_PREFIX_."product_supplier WHERE id_product='".(int)$prodrow['id_product']."' AND id_supplier='".(int)$prodrow['id_supplier']."'");
						...
						break;
					case'ean13':case'location':case'out_of_stock':case'reference':case'meta_title':case'meta_description':case'meta_keywords':
						echo "<cell><![CDATA[".$prodrow[$col]."]]></cell>";
						break;
					case'quantity':
					...
					case'unit_price_ratio':
						echo "<cell>".number_format($prodrow['price']/($prodrow[$col] ? $prodrow[$col] : $prodrow['price']), 6, '.', '')."</cell>";
						break;	
					case'date_add':
