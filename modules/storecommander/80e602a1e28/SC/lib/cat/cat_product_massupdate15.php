							...
							if(!$product->hasAttributes())
							{
								...
								foreach($shops as $shop_id)
								{
									...
									if(!empty($id_stock_available))
										$stock_available = StockAvailable::updateQuantity($product_id, null, $todo, $shop_id);
									else
							...
							if(!$product->hasAttributes())
							{
								...
								foreach($shops as $shop_id)
								{
									...
									if(!empty($id_stock_available))
									{
										...
										$stock_available = StockAvailable::updateQuantity($product_id, null, $qty, $shop_id);
									}
								}
					...
					if (strpos($todo,'%')===false)
					{
						foreach($product_attrs as $product_attr)
						{
							...
							foreach($shops as $shop_id)
							{
								...
								if(!empty($id_stock_available))
									$stock_available = StockAvailable::updateQuantity($product_attr["id_product"], $product_attr["id_product_attribute"], $todo, $shop_id);
								else
						...
						foreach($product_attrs as $product_attr)
						{
							...
							foreach($shops as $shop_id)
							{
								...
								if(!empty($id_stock_available))
								{
									...
									$stock_available = StockAvailable::updateQuantity($product_attr["id_product"], $product_attr["id_product_attribute"], $qty, $shop_id);
								}
							}
						}
					}
			...
			case'defaultcombination':
				...
				switch($todo)
				{
					case'cheapest':
						...
						{
							Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product_attribute_shop SET default_on=0 WHERE id_product_attribute IN (SELECT id_product_attribute FROM '._DB_PREFIX_.'product_attribute WHERE id_product = '.(int)$id_product.')');
							$ida=Db::getInstance()->getValue('SELECT pas2.id_product_attribute FROM '._DB_PREFIX_.'product_attribute_shop pas2 WHERE pas2.id_product_attribute IN (SELECT pa.id_product_attribute FROM '._DB_PREFIX_.'product_attribute pa WHERE pa.id_product = '.(int)$id_product.') ORDER BY pas2.price ASC');
							Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product_attribute_shop SET default_on=1 WHERE id_product_attribute = '.(int)$ida);
						}
						break;
					case'instockandcheapest':
						...
						foreach($productlist AS $id_product)
						{
							$res = Db::getInstance()->ExecuteS('SELECT pa.id_product,pas2.id_product_attribute FROM '._DB_PREFIX_.'product_attribute_shop pas2 
																									...
																									WHERE pas2.id_product_attribute IN (SELECT pa.id_product_attribute FROM '._DB_PREFIX_.'product_attribute pa WHERE pa.id_product = '.(int)$id_product.') ORDER BY pas2.price ASC');
							foreach($res as $row)
							{
								$qty = SCI::_getProductAttributeQty($row['id_product'],$row['id_product_attribute']);
								if ($qty > 0)
								{
									Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product_attribute_shop SET default_on=0 WHERE id_product_attribute IN (SELECT id_product_attribute FROM '._DB_PREFIX_.'product_attribute WHERE id_product = '.(int)$row['id_product'].')');
									Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product_attribute_shop SET default_on=1 WHERE id_product_attribute = '.(int)$row['id_product_attribute']);
									break;
								}
							}
						}
