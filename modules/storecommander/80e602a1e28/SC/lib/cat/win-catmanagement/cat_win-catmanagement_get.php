		...
		if(!empty($res))
		{
			foreach($res as $k => $row){
				if(!empty($row["id_category"]))
				{
					...
					$is_home = false;
					if(SCMS && in_array($row['id_category'], $root_cat))
						$is_home = true;
...
{
	global $id_lang,$id_shop,$root_cat,$array_cats,$array_children_cats;
	if(!empty($array_children_cats[$id_parent]))
	{
		...
		foreach($array_children_cats[$id_parent] as $k => $id)
		{
			...
			{
				if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
					$image = '<img src="'.SC_PS_PATH_REL.'img/c/br.jpg" height="'.getGridImageHeight().'px" alt="" />';
				else
