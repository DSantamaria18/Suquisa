			...
			function duplicateCategories($id_category, $id_parent)
			{
				...
				if(!empty($result[0]["id_category"]))
				{
					...
					if(isset($fields["level_depth"]))
						$fields["level_depth"]=$category_parent->level_depth;
							...
							if(copy ( $actual_image_name , $new_image_name ))
							{
								...
								foreach ($images_types as $k => $image_type)
								{
									if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
									{
										...
										imageResize($new_image_name, _PS_CAT_IMG_DIR_.$new_id_cat.'-'.stripslashes($imageType['name']).'.jpg', (int)($imageType['width']), (int)($imageType['height']));
								}
							}
						}
