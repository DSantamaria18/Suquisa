...
/*					case'price_public': // SPECIAL
					...
					case'price':
						if (version_compare(_PS_VERSION_, '1.4.0.0', '>='))
						{
							...
							if(!empty($taxrate))
								echo ("<cell>".number_format($product_attribute['price']*($taxrate/100+1)+$product_attribute['pprice']*($taxrate/100+1) + $ecotax, 6, '.', '')."</cell>");
							else
...
/*	function getColIndex($col)
	...
	function getColSettingsAsXML()
	{
		...
		foreach ($cols as $col)
		{
			if(!in_array($col, array('ATTR','quantityupdate','pprice','ppriceextax','priceextax','margin','pweight','quantity_physical','quantity_usable','quantity_real','available_later')))
				$pa_cols[] = $col;
		}
...
*/
		...
		foreach($results as $combinaison)
		{
			...
			if (!in_array($combinaison['group_name'],$attributeGroupsNames)){
				if(!empty($combinaison['id_attribute_group']))
				{
