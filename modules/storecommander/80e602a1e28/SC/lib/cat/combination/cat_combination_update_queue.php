			...
			if(!empty($log_ids[$num]))
			{
				...
				if(!empty($action) && $action=="delete" && !empty($gr_id))
				{
					...
					if(!empty($id_product))
					{
						...
						if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
						{
							...
							$p = new Product($id_product, false, (int)$lang, (int)SCI::getSelectedShop());
							...
							if (!$p->hasAttributes())
							{
								...
								Db::getInstance()->Execute($sql);
								$sql="UPDATE "._DB_PREFIX_."product_shop SET cache_default_attribute='0' WHERE id_product='".intval($id_product)."' AND id_shop IN (".SCI::getSelectedShopActionList().") ";
								Db::getInstance()->Execute($sql);
							}
							else
							{
				...
				elseif(!empty($action) && $action=="update" && !empty($gr_id))
				{
					...
					if(!empty($id_product))
					{
						...
						if (version_compare(_PS_VERSION_, '1.4.0.0', '>=') && isset($_POST['default_on']) && intval($_POST['default_on'])==1)
						{
							...
							if ($quantityUpdate!=0)
							{
								if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
								{
									foreach(SCI::getSelectedShopActionList(false, $id_product) AS $id_shop)
									{
										...
										if(!empty($exist))
											StockAvailable::updateQuantity((int)$id_product, (int)$id_product_attribute, $quantityUpdate, (int)$id_shop);
										else
						...
						if (isset($_POST['price']) || isset($_POST['priceextax'])) // need tax rate?
						{
						...
						if (isset($_POST['priceextax'])) // excluding tax should be placed before including taxe for price round.
						{
							if (version_compare(_PS_VERSION_, '1.4.0.0', '>='))
							{
						...
						if (isset($_POST['ecotax']) && isset($_POST['ecotaxentered']) && $_POST['ecotaxentered']==1)
						{
						...
						if (isset($_POST['price']) && version_compare(_PS_VERSION_, '1.4.0.0', '<'))
						{
						...
						if (isset($_POST['available_later']) && SCI::getConfigurationValue("SC_DELIVERYDATE_INSTALLED")=="1")
						{
