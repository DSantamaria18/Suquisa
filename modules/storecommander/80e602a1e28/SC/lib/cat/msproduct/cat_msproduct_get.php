	...
	function generateValue($col, $prodrow)
	{
		...
		switch($col){
			case'wholesale_price':
				...
				break;
			case'quantity':
				$return .= "<cell>".SCI::getProductQty((int)$prodrow['id'])."</cell>";
				break;
			case'advanced_stock_management':
				...
				break;
			case'ecotax':
				$return .= "<cell>".number_format( $prodrow['ecotax'] * SCI::getEcotaxTaxRate() , 2, '.', '')."</cell>";
				break;
			case'price':
				$return .= "<cell".(sc_array_key_exists('wholesale_price',$prodrow) && $prodrow['wholesale_price'] > $prodrow['price']?' bgColor="#FF0000"  style="color:#FFFFFF"':'').">".number_format($prodrow['price'], 2, '.', '')."</cell>";
				break;
			...
			case'link_rewrite':
				...
				break;
			case'price_inc_tax':
				$return .= "<cell>".number_format($prodrow['price']*($tax[intval($prodrow['id_tax_rules_group'])]/100+1)+( $prodrow['ecotax']*SCI::getEcotaxTaxRate() ), 2, '.', '')."</cell>";
				break;
			case'margin':
				$return .= '<cell></cell>';
				break;
