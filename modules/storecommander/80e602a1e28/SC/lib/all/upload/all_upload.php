...
if(($obj!='importcsv' && $obj!='importcsvcus') && $chunks < 2 && file_exists($targetDir . $fileName)){
...
function getUpload()
{
	...
	switch($obj)
	{
		...
		case 'attachment':
			...
			{
				$sqlstr.='('.intval($id_attachment).','.intval($lang['id_lang']).',\''.psql($name).'\',\''.psql($name).'_'.psql($lang['iso_code']).'\'),';
			}
			$sqlstr = trim($sqlstr,',');
