 ...
 **/
	...
	function getRowsFromDB(){
		global $id_lang,$id_segment,$filters;
		...
		$sql = '
		SELECT ct.*, CONCAT(c.`firstname`," ",c.`lastname`) as customer, c.id_customer, cl.`name` as contact, cl.id_contact,
 			(
				SELECT cm2.`id_employee`
		...
		foreach ($res AS $row)
		{
			...
			$xml.=("<row id='".$row['id_customer_thread']."'>");
				...
				$xml.=("<cell><![CDATA[".$employee->firstname." ".$employee->lastname."]]></cell>");
