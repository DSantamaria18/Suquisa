 ...
 **/
	...
	switch($action){
		...
		case 'mapping_process':
			...
			$addressFields=array('address_title','address_country','address_state','address_company','address_lastname','address_firstname','address_1','address_2','address_postcode','address_city','address_other','address_phone','address_phonemobile');
			...
			switch ($importConfig[$filename]['idby']){
				case'idcustomer':
					if (!in_array('id_customer',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_customer field'));
					break;
				case'email':
					if (!in_array('email',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the email field'));
					break;
				case'idcustomeradresse':
					if (!in_array('id_customer',$mappingData['DBArray']) && !in_array('address_title',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_customer and the address title fields'));
					else if (!in_array('id_customer',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_customer field'));
					else if (!in_array('address_title',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the address title field'));
					break;
				case'emailadresse':
					if (!in_array('email',$mappingData['DBArray']) && !in_array('address_title',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the email and the address title fields'));
					else if (!in_array('email',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the email field'));
					else if (!in_array('address_title',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the address title field'));
					break;
				case'idcustomeridadresse':
					if (!in_array('id_customer',$mappingData['DBArray']) && !in_array('id_address',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_customer and the id_address fields'));
					else if (!in_array('id_customer',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_customer field'));
					else if (!in_array('id_address',$mappingData['DBArray']))
						die(_l('Wrong mapping, mapping should contain the id_address field'));
					break;
				case'emailidadresse':
					if (!in_array('email',$mappingData['DBArray']) && !in_array('id_address',$mappingData['DBArray']))
			...
			for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA)-1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'],$DATA[$current_line])) && ($current_line <= $importlimit)) ; $current_line++)
			{
				...
				foreach($line AS $key => $value)
				{
					...
					if (sc_array_key_exists($key,$firstLineData) && in_array($firstLineData[$key],$mappingData['CSVArray']))
					{
						...
						{
							// CUSTOMER
							case 'company':$newcustomer->company=psql($value);break;
							case 'siret':$newcustomer->siret=psql($value);break;
							case 'ape':$newcustomer->ape=psql($value);break;
							case 'firstname':$newcustomer->firstname=psql($value);break;
							case 'lastname':$newcustomer->lastname=psql($value);break;
							case 'email':$newcustomer->email=psql($value);break;
							case 'passwd':$newcustomer->passwd=psql(Tools::encrypt($value));break;
							case 'birthday':$newcustomer->birthday=psql(importConv2Date($value));break;
							case 'date_add':$newcustomer->date_add=psql(importConv2Date($value));break;
							...
							case 'website':$newcustomer->website=psql($value);break;
							case 'active':$newcustomer->active=intval(getBoolean($value));break;
							case 'note':$newcustomer->note=psql($value);break;
							case 'id_gender':
							...
							// ADDRESS
							case 'address_title':$newaddress->alias=psql($value);break;
							...
							break;
							case 'address_company':$newaddress->company=psql($value);break;
							case 'address_lastname':$newaddress->lastname=psql($value);break;
							case 'address_firstname':$newaddress->firstname=psql($value);break;
							case 'address_1':$newaddress->address1=psql($value);break;
							case 'address_2':$newaddress->address2=psql($value);break;
							case 'address_postcode':$newaddress->postcode=psql($value);break;
							case 'address_city':$newaddress->city=psql($value);break;
							case 'address_other':$newaddress->other=psql($value);break;
							case 'address_phone':$newaddress->phone=psql($value);break;
							case 'address_phonemobile':$newaddress->phone_mobile=psql($value);break;
				...
				if ($newcustomer->save())
				{
					...
					if(in_array("groups",$mappingData['DBArray']) && !empty($name_groups))
					{
						$temp_groups = array();
