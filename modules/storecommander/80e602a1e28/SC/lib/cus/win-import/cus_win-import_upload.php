...
<?php
echo (in_array($user_lang_iso,array('cs','da','de','es','fi','fr','hr','hu','it','ja','lv','nl','pt','br','ro','ru','sr','sv'))?'<script type="text/javascript" src="'.SC_PLUPLOAD.'js/i18n/'.$user_lang_iso.'.js"></script>':'');
?>
<script type="text/javascript">
	...
	$(window).ready(function(){
		noerror=true;
		$("#uploaderImportCusCSV").pluploadQueue({
			// General settings
			runtimes : 'gears,html5,flash,silverlight,browserplus',
			url : 'index.php?ajax=1&act=all_upload&obj=importcsvcus',
			max_file_size : '30mb',
			chunk_size : '500ko',
			unique_names : false,
			...
			init : {
	            BeforeUpload : function(up){
					$("#uploaderImportCusCSV").pluploadQueue().settings.url='index.php?ajax=1&act=all_upload&obj=importcsvcus';
	            },
	            UploadComplete : function(up){
					top.wImportCus._uploadWindow.hide();
					top.displayOptions();
	            }
			},
      // Flash settings  
      ...
      silverlight_xap_url : '<?php echo SC_PLUPLOAD;?>js/plupload.silverlight.xap' 
		...
		$('form').submit(function(e){
			var uploader = $('#uploaderImportCusCSV').pluploadQueue();
...
#divAddImportCusCSV{ width:inherit;height:inherit;}
#uploaderImportCusCSV{ width:inherit;height:inherit;}
.formDragDrop{ width:inherit;height:inherit;margin:0;}
</style>	

<div id="divAddImportCusCSV">
	<form class="formDragDrop">
		<div id="uploaderImportCusCSV">
