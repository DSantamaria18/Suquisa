 ...
 **/
	...
	// Groupes
	$arrGroupes=array();
	if (in_array('id_default_group',$cols) || in_array('groups',$cols))
	{
		$arrGroupes[0]='-';
		$inner = "";
	...
	function getCustomers()
	{
		...
		if ($view=='grid_address' || in_array("id_address", $cols))
		{
			...
			foreach($cols as $col)
			{
				/*if (in_array($col,$blacklistfields)) // calculated fields
				 continue;*/
				if (in_array($col,$fields_address))
					$sqlAddress.=',a.`'.$col.'`';
			}
			...
			if(!empty($filter_params))
			{
				...
				foreach ($filters as $filter)
				{
					...
					{
						if (in_array($field,array('id_customer','id_order')))
						{
		...
		{
			$fields_customer=array('id_shop','id_customer','id_gender','company','siret','ape','firstname','lastname','email','active','newsletter','optin','date_add','birthday','id_default_group','note');
			$fields_lang=array();
			sc_ext::readCustomCustomersGridsConfigXML('updateSettings');
			$blacklistfields=array();
