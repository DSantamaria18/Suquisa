...
?>
	...
	prop_tb.addButtonSelect('panel',0,'<?php echo _l('Order history',1)?>',icons,'lib/img/text_list_numbers.png','lib/img/text_list_numbers.png',false,true);
	...
	function setPropertiesPanel(id){
		if (id=='help'){
			<?php echo "window.open('".getHelpLink('ord_toolbar_prod_prop')."');"; ?>
		}
		prop_tb._sb.setText('');
...
<?php
	...
	foreach ($files as $item)
		if ($item != '.' && $item != '..')
			if (is_dir(SC_DIR.'lib/ord/'.$item) && file_exists(SC_DIR.'lib/ord/'.$item.'/ord_'.$item.'_init.php') && substr($item,0,4)!='win-')
