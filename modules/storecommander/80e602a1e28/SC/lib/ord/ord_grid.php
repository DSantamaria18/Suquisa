	...
	function gridToolBarOnClick(id){
			...
			if (id.substr(0,8)=='download'){
				...
				if (selection!='' && selection!=null)
				{
					...
					{
						selOrd[i]=ord_grid.cells(selArray[i],idxIdOrder).getValue();
					}
...
?>
	...
	function onGridOrdContextButtonClick(itemId){
		...
		if (itemId=="gopsbo"){
			...
			wModifyOrder.attachURL("<?php echo SC_PS_PATH_ADMIN_REL;?>index.php?tab=AdminOrders&vieworder&id_order="+tabId+"&token=<?php echo $sc_agent->getPSToken('AdminOrders');?>");
...
function groupByProduct()
{
	...
	if (ord_grid_groupByProduct=="1")
	{
		...
		else if(idxProductId!=undefined)
...
function displayOrders(callback)
{
	...
	if(id_selected_segment!=undefined && id_selected_segment!=null && id_selected_segment!=0)
		loadUrl = "index.php?ajax=1&act=ord_order_get&id_segment="+id_selected_segment+"&filter_params="+filter_params+"&view="+gridView+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime();
	<?php } ?>
