...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="jq-content-layout">';
        $result .= '<div class="jq-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="jq-layout-cell' . ('jq-block' == $style ? ' jq-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="jq-post">' string in it.
     * Then it wraps the content of the buffer with the jq-post.
     */
    public function componentWrapper()
    {
        ...
        if (false === strpos($content, '<div class="jq-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'jq-main').addClass('jq-postcontent');
            }
        });
    }
});
