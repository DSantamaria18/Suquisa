...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the jq-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"jq-post" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"jq-postheader\">");
            ...
            artxFragmentEnd("</h2>");
            artxFragmentBegin("<div class=\"jq-postheadericons jq-metadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"jq-postcontent clearfix\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"jq-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"jq-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"jq-blockcontent\">");
     ...
     *  Select inputs with class != 'jq-button'
     *   artxFindTags($html, array('input' => array('class' => '!jq-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'jq-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!jq-button', '!'))))
     *  Select inputs with class = button but != 'jq-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!jq-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'jq-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'jq-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!jq-button')),
            'button' => array('class' => array('button', '!jq-button')), 'button' => array('class' => array('btn', '!jq-button'))));
        return $content;
    }
