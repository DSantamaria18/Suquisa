...
// 		$format = (VmConfig::get('order_html_email',1)) ? 'html' : 'raw';
	...
	static public function getManufacturerDesc($product_id){
	
		$q = 'SELECT mf_desc FROM `jq_virtuemart_manufacturers_es_es`,`jq_virtuemart_product_manufacturers` WHERE `jq_virtuemart_manufacturers_es_es`.virtuemart_manufacturer_id = `jq_virtuemart_product_manufacturers`.virtuemart_manufacturer_id and `jq_virtuemart_product_manufacturers`.virtuemart_product_id = ' . $product_id;
		$db = JFactory::getDbo();
		$db->setQuery ($q);
		if ($parent = $db->loadObject()){
			$retorno = $parent->mf_desc;
