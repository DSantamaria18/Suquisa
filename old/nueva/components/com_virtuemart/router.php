...
class vmrouterHelper {
	...
	/* Get Manufacturer Desc from Product */
	public function getManufacturerDesc($id){

		static $productNamesCache = array();

		if(!isset($productNamesCache[$id])){
			$db = JFactory::getDBO();
			$query = 'SELECT mf_desc FROM  `jq_virtuemart_manufacturers_es_es` ,  `jq_virtuemart_product_manufacturers` ' .
                     ' WHERE  `jq_virtuemart_manufacturers_es_es`.virtuemart_manufacturer_id = `jq_virtuemart_product_manufacturers`.virtuemart_manufacturer_id ' .
                     ' AND  `jq_virtuemart_product_manufacturers`.virtuemart_product_id =1 = ' . (int) $id;
			$db->setQuery($query);
			$name = $db->loadResult();
			$productNamesCache[$id] = $name ;
		} else {
		...
		return $name.$this->manufacturerDesc;
	}
